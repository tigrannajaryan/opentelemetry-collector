...
func (orig *Span) LazyMessage() *proto.LazyMessage {
	return &orig.lazy
}

// SetLazyParent records the parent lazy state used for modification bubbling.
func (orig *Span) SetLazyParent(parent *proto.LazyMessage) {
...
func (orig *Span) UnmarshalProto(buf []byte) error {
	if err := validateSpanProto(buf); err != nil {
		return err
	}
	orig.Reset()
...
func (orig *Span) DecodeAll() {
	...
	for i := range orig.Events {
		if orig.Events[i] != nil {
			orig.Events[i].DecodeAll()
		}
	}

	for i := range orig.Links {
		if orig.Links[i] != nil {
			orig.Links[i].DecodeAll()
		}
	}

	orig.Status.DecodeAll()
	orig.lazy.Clear()
}

func validateSpanProto(buf []byte) error {
	var err error
	var fieldNum int32
	...
	for pos < l {
		...
		case 11:
			...
			startPos := pos - length
			err = validateSpanEventProto(buf[startPos:pos])
			if err != nil {
				return err
			}
		...
		case 13:
			...
			startPos := pos - length
			err = validateSpanLinkProto(buf[startPos:pos])
			if err != nil {
				return err
			}
		...
		case 15:
			...
			startPos := pos - length
			err = validateStatusProto(buf[startPos:pos])
			if err != nil {
				return err
			}
		default:
...
func (orig *Span) decodeProto(buf []byte) error {
	var err error
	var fieldNum int32
	var wireType proto.WireType
	...
	for pos < l {
		...
		case 11:
			...
			startPos := pos - length
			orig.Events = append(orig.Events, NewSpanEvent())
			orig.Events[len(orig.Events)-1].lazy.Init(buf[startPos:pos], &orig.lazy)
		...
		case 13:
			...
			startPos := pos - length
			orig.Links = append(orig.Links, NewSpanLink())
			orig.Links[len(orig.Links)-1].lazy.Init(buf[startPos:pos], &orig.lazy)
		...
		case 15:
			...
			(&orig.Status).lazy.Init(buf[startPos:pos], &orig.lazy)
		default:
			pos, err = proto.ConsumeUnknown(buf, pos, wireType)
			if err != nil {
				return err
