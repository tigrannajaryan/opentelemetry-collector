...
func (orig *Resource) LazyMessage() *proto.LazyMessage {
	return &orig.lazy
}

// SetLazyParent records the parent lazy state used for modification bubbling.
func (orig *Resource) SetLazyParent(parent *proto.LazyMessage) {
...
func (orig *Resource) UnmarshalProto(buf []byte) error {
	if err := validateResourceProto(buf); err != nil {
		return err
	}
	orig.Reset()
...
func (orig *Resource) EnsureDecoded() {
...
func (orig *Resource) MarkModified() {
	orig.EnsureDecoded()
...
func (orig *Resource) DecodeAll() {
	...
	for i := range orig.EntityRefs {
		if orig.EntityRefs[i] != nil {
			orig.EntityRefs[i].DecodeAll()
		}
	}
	orig.lazy.Clear()
}

func validateResourceProto(buf []byte) error {
	var err error
	var fieldNum int32
	...
	for pos < l {
		...
		case 3:
			...
			startPos := pos - length
			err = validateEntityRefProto(buf[startPos:pos])
			if err != nil {
				return err
			}
		default:
...
func (orig *Resource) decodeProto(buf []byte) error {
	var err error
	var fieldNum int32
	var wireType proto.WireType
	...
	for pos < l {
		...
		case 3:
			...
			startPos := pos - length
			orig.EntityRefs = append(orig.EntityRefs, NewEntityRef())
			orig.EntityRefs[len(orig.EntityRefs)-1].lazy.Init(buf[startPos:pos], &orig.lazy)
		default:
			pos, err = proto.ConsumeUnknown(buf, pos, wireType)
			if err != nil {
				return err
