...
func (orig *Location) LazyMessage() *proto.LazyMessage {
	return &orig.lazy
}

// SetLazyParent records the parent lazy state used for modification bubbling.
func (orig *Location) SetLazyParent(parent *proto.LazyMessage) {
...
func (orig *Location) UnmarshalProto(buf []byte) error {
	if err := validateLocationProto(buf); err != nil {
		return err
	}
	orig.Reset()
...
func (orig *Location) EnsureDecoded() {
...
func (orig *Location) MarkModified() {
...
func (orig *Location) DecodeAll() {
	orig.EnsureDecoded()

	for i := range orig.Lines {
		if orig.Lines[i] != nil {
			orig.Lines[i].DecodeAll()
		}
	}
...
func validateLocationProto(buf []byte) error {
	var err error
	var fieldNum int32
	...
	for pos < l {
		...
		case 3:
			...
			startPos := pos - length
			err = validateLineProto(buf[startPos:pos])
			if err != nil {
				return err
			}
		case 4:
...
func (orig *Location) decodeProto(buf []byte) error {
	var err error
	var fieldNum int32
	var wireType proto.WireType
	...
	for pos < l {
		...
		case 3:
			...
			startPos := pos - length
			orig.Lines = append(orig.Lines, NewLine())
			orig.Lines[len(orig.Lines)-1].lazy.Init(buf[startPos:pos], &orig.lazy)
		case 4:
			switch wireType {
			case proto.WireTypeLen:
				var length int
