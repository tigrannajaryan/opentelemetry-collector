...
func (orig *ScopeProfiles) LazyMessage() *proto.LazyMessage {
	return &orig.lazy
}

// SetLazyParent records the parent lazy state used for modification bubbling.
func (orig *ScopeProfiles) SetLazyParent(parent *proto.LazyMessage) {
...
func (orig *ScopeProfiles) UnmarshalProto(buf []byte) error {
	if err := validateScopeProfilesProto(buf); err != nil {
		return err
	}
	orig.Reset()
...
func (orig *ScopeProfiles) EnsureDecoded() {
...
func (orig *ScopeProfiles) MarkModified() {
...
func (orig *ScopeProfiles) DecodeAll() {
	orig.EnsureDecoded()
	orig.Scope.DecodeAll()
	for i := range orig.Profiles {
		if orig.Profiles[i] != nil {
			orig.Profiles[i].DecodeAll()
		}
	}
...
func validateScopeProfilesProto(buf []byte) error {
	var err error
	var fieldNum int32
	...
	for pos < l {
		...
		case 2:
			...
			startPos := pos - length
			err = validateProfileProto(buf[startPos:pos])
			if err != nil {
				return err
			}
...
func (orig *ScopeProfiles) decodeProto(buf []byte) error {
	var err error
	var fieldNum int32
	var wireType proto.WireType
	...
	for pos < l {
		...
		case 2:
			...
			startPos := pos - length
			orig.Profiles = append(orig.Profiles, NewProfile())
			orig.Profiles[len(orig.Profiles)-1].lazy.Init(buf[startPos:pos], &orig.lazy)
