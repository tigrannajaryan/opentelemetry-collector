...
func (orig *Sample) LazyMessage() *proto.LazyMessage {
	return &orig.lazy
}

// SetLazyParent records the parent lazy state used for modification bubbling.
func (orig *Sample) SetLazyParent(parent *proto.LazyMessage) {
...
func (orig *Sample) UnmarshalProto(buf []byte) error {
	if err := validateSampleProto(buf); err != nil {
		return err
	}
	orig.Reset()
...
func (orig *Sample) EnsureDecoded() {
...
func (orig *Sample) DecodeAll() {
	orig.EnsureDecoded()

	orig.lazy.Clear()
}

func validateSampleProto(buf []byte) error {
	var err error
	var fieldNum int32
	...
	for pos < l {
		...
		case 5:
			...
			case proto.WireTypeLen:
				...
				if length%8 != 0 {
					return fmt.Errorf("proto: invalid field len = %d for field TimestampsUnixNano", length)
				}
...
func (orig *Sample) decodeProto(buf []byte) error {
	var err error
	var fieldNum int32
	var wireType proto.WireType
