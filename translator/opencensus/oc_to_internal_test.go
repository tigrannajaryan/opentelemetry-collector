...
func TestOcNodeResourceToInternal(t *testing.T) {
	...
	expectedAttrs := data.AttributesMap{
		...
		ocAttributeProcessID:                 data.NewAttributeValueInt(123),
		ocAttributeProcessStartTime:          data.NewAttributeValueString("2020-02-11T20:26:00Z"),
		conventions.AttributeLibraryLanguage: data.NewAttributeValueString("CPP"),
		ocAttributeExporterVersion:           data.NewAttributeValueString("v1.2.0"),
		...
		ocAttributeResourceType:              data.NewAttributeValueString("good-resource"),
		"resource-attr":                      data.NewAttributeValueString("val2"),
	}

	assert.EqualValues(t, expectedAttrs, resource.AttributesMap())
	...
	ocResource.Labels[ocAttributeResourceType] = "this will be overridden 2"
...
func TestOcToInternal(t *testing.T) {
	...
	span1.SetStartTime(timestampToUnixnano(timestampP))
	span1.SetEndTime(timestampToUnixnano(timestampP))
	span1.SetEvents([]*data.SpanEvent{
		data.NewSpanEvent(
			timestampToUnixnano(timestampP),
			"event1",
			data.NewAttributes(
				data.AttributesMap{
	...
	span1.SetStatus(data.NewSpanStatus(otlptrace.Status_Cancelled, "status-cancelled"))

	span2 := data.NewSpan()
	span2.SetName("operationC")
	span2.SetStartTime(timestampToUnixnano(timestampP))
	span2.SetEndTime(timestampToUnixnano(timestampP))
	span2.SetLinks([]*data.SpanLink{data.NewSpanLink()})
	span2.SetDroppedLinksCount(1)

	span3 := data.NewSpan()
	span3.SetName("operationD")
	span3.SetStartTime(timestampToUnixnano(timestampP))
	span3.SetEndTime(timestampToUnixnano(timestampP))
	...
	tests := []struct {
...

		{
			...
			itd: data.NewITraceData([]*data.ResourceSpans{
				data.NewResourceSpans(&data.Resource{}, nil),
			}),
		},
...

		{
			...
			itd: data.NewITraceData([]*data.ResourceSpans{
				data.NewResourceSpans(&data.Resource{}, []*data.Span{span1}),
			}),
		},
...

		{
			...
			itd: data.NewITraceData([]*data.ResourceSpans{
				data.NewResourceSpans(&data.Resource{}, []*data.Span{span1, span2}),
				data.NewResourceSpans(&data.Resource{}, []*data.Span{span3}),
			}),
		},
